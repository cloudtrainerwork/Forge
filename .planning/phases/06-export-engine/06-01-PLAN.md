...
<task type="auto">
  <name>Task 0: Create test scaffolds for GSD entities and generator</name>
  <files>src/domain/entities/GSDPlan.test.ts, src/services/GSDXmlGenerator.test.ts</files>
  <action>
    Create test scaffold files for domain entities and XML generator service:
  ...
  <verify>
    <automated>npm test src/domain/entities/GSDPlan.test.ts && npm test src/services/GSDXmlGenerator.test.ts</automated>
  </verify>
  <done>Test scaffold files exist and can be executed without errors</done>
</task>
